...
// Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
// Defect(s) Found: People with finite turns were not re-enqueued correctly; queue did not cycle properly.
public void TestTakingTurnsQueue_FiniteRepetition()*/
...
public class TakingTurnsQueueTests
{
    ...
    public void TestTakingTurnsQueue_ForeverZero()
    {
        ...
        // Verify infinite turns still have original value
        var infinitePerson = players.GetNextPerson();
        Assert.AreEqual(timTurns, infinitePerson.Turns,
            "People with infinite turns should not have their turns parameter modified.");
    }
